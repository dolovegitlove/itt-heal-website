...
(function() {
    ...
    const BookingAvailability = {
        ...
        loadTimeSlots() {
            ...
            // Temporarily bypass frontend date validation - let backend handle it
            // Backend already validates business days and advance booking requirements
            // if (!this.isValidBookingDate(selectedDate)) {
            ...
            // Build proper URL - handle /3t/ subdirectory context
            const baseUrl = window.location.origin;
            const apiUrl = `${baseUrl}/api/web-booking/availability/${practitionerId}/${selectedDate}?service_type=${serviceType}`;
            ...
            fetch(apiUrl, {
                ...
                .then(data => {
                    ...
                    if (data.success && data.data && data.data.available_slots && data.data.available_slots.length > 0) {
                        // Success - populate time slots
                        console.log('✅ Populating time slots:', data.data.available_slots.length);
                        this.populateTimeSlots(data.data.available_slots, timeSelect, selectedDate);
                        if (loadingDiv) {
                            loadingDiv.style.display = 'none';
                        }
                    } else {
                        // No available slots - show what we actually got
                        console.log('❌ No available slots in response');
                        timeSelect.innerHTML = `<option value="">Debug: success=${data.success}, slots=${data.data?.available_slots?.length || 0}</option>`;
                        timeSelect.disabled = true;
                        if (loadingDiv) {
                            loadingDiv.style.display = 'none';
                        }
