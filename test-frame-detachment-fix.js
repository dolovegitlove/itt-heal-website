...
class FrameDetachmentTest {
        ...
        try {
            ...
            if (testButton) {
                ...
                if (nextBtn) {
                    ...
                    await nextBtn.click();
                    await new Promise(resolve => setTimeout(resolve, 2000)); // Wait for async transition
                    ...
                    if (dateField) {
                        ...
                        if (nextBtn2) {
                            ...
                            if (nextBtn3) {
                                ...
                                await new Promise(resolve => setTimeout(resolve, 4000)); // Extended wait for Stripe initialization
                                ...
                                await new Promise(resolve => setTimeout(resolve, 2000)); // Additional wait for Stripe
                                ...
                                // Test payment intent creation without frame detachment
                                try {
                                        ...
                                        try {
                                            const response = await fetch('/api/web-booking/create-payment-intent', {
                                                ...
                                                body: JSON.stringify({
                                                    ...
                                                    service_type: 'test',
                                                    client_info: { 
                                                        name: 'Frame Test User', 
                                                        email: 'frametest@example.com',
                                                        phone: '5551234567'
                                                    }
                                                })
                                            });
                                    ...
                                    this.addResult('Payment Intent API', paymentIntentTest.status === 200 && paymentIntentTest.hasClientSecret, 
                                        `API Status: ${paymentIntentTest.status}, Client Secret: ${paymentIntentTest.hasClientSecret ? 'Present' : 'Missing'}`);
    ...
    async testBackNavigation() {
        this.log('🔹', 'Testing back navigation for frame stability...');
        
        try {
            // Navigate forward through steps
            await this.page.click('div[onclick*="selectService(\'test\'"]');
            await new Promise(resolve => setTimeout(resolve, 1000);
            ...
            if (dateField) {
                ...
                const prevBtn = await this.page.$('#prev-btn');
                if (prevBtn) {
                    await prevBtn.click();
                    await new Promise(resolve => setTimeout(resolve, 2000);
                    
                    const frameValid = await this.validateFrameIntegrity();
                    this.addResult('Back Navigation Frame Integrity', frameValid, 'Frame stable during backward navigation');
                }
            }
            
        } catch (error) {
            this.addResult('Back Navigation Test', false, `Back navigation test failed: ${error.message}`);
        }
    }
    ...
    async runTests() {
        try {
            ...
            // Reset page for back navigation test
